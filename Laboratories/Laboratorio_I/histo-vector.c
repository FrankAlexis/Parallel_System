...
void process_all_sq (char** all, size_t sq_num, int k_mers, unsigned short* histogram, int Nthreads);
...
int main(int argc, char *argv[])
{
  if (argc != 5)
  {
    fprintf(stderr, "ERROR - usage: histo <file> k_mers <outfile> threads\n");
    exit(1);
  }
#ifdef DEBUG
  ...
  process_all_sq(all_sq, n_seq, k_mers, histogram, Nthreads);
...
void process_all_sq(char **all, size_t sq_num, int k_mers, unsigned short *histogram, int Nthreads){
  int i, j, sq_len;
  unsigned long in;
  #pragma omp parallel for num_threads(Nthreads) default(none) private(j, in, sq_len) shared(histogram, i, all, sq_num, k_mers) schedule(auto)
  for (i = 0; i < sq_num; i++){
    sq_len = strlen(all[i]);
